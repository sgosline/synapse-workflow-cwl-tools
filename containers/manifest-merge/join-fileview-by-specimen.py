...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--filelist', type=str, nargs='+', required=True)
    parser.add_argument('-p', '--parentId', type=str, required=True)
    parser.add_argument('-v', '--values', type=str, nargs='+', required=True,
                        help='Values by which you would like to join the synapse ids')
    parser.add_argument('-k', '--key', type=str, default='specimenID',
                        help='Key on which you want to join.')
    parser.add_argument('-u', '--used', type=str, nargs='+', required=False,
                        help='List of synapse ids used in this analysis')
    parser.add_argument('-e', '--executed', type=str, nargs='+', required=False,
                        help='List of files used to execute this')

    args = parser.parse_args()

    if len(args.values) != len(args.filelist):
        raise ValueError('Values and file list need to be the same length.')

    # read in manifest
    manifest = pandas.read_csv(args.manifest_file, sep='\t')

    # join specimens and synids into data frame
    data = {args.key: args.values, 'path': [os.path.basename(a) for a in args.filelist]}
    specToSyn = pandas.DataFrame(data)
